...
- `index.html` is the only entrypoint. It loads local vendored runtime assets plus `main.js`; `init()` in `main.js` initializes i18next, registers the service worker, configures PDF.js, renders the current page, binds the UI, and schedules OCR engine preload.
